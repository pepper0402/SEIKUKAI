          ...
          {viewMode === 'current' ? (
            currentCriteria.map((c) => (
              <div key={c.id} className="bg-white rounded-[28px] p-4 shadow-sm border border-gray-50 flex items-center gap-4 group">
                ...
                {c.video_url && (
                  <a href={c.video_url} target="_blank" rel="noreferrer" className="w-9 h-9 bg-red-50 text-red-500 rounded-xl flex items-center justify-center hover:bg-red-600 hover:text-white transition-all">▶️</a>
                )}
              </div>
            ))
          ) : (
            historyData.length === 0 ? (
              <div className="bg-white rounded-[28px] p-10 text-center text-gray-300 font-black italic border border-dashed border-gray-200">NO HISTORY DATA</div>
            ) : (
              historyData.map((h) => (
                <div key={h.id} className="bg-white rounded-[28px] p-4 shadow-sm border border-gray-50 flex items-center gap-4">
                  <div className="shrink-0 w-12 h-12 rounded-[18px] bg-gray-50 flex items-center justify-center font-black text-lg text-gray-800 border-2 border-gray-200">
                    {h.grade}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[8px] font-black text-orange-500 uppercase leading-none mb-1">{h.criteria?.dan || '過去の級'}</p>
                    <p className="text-[12px] font-bold text-[#001f3f] leading-tight">{h.criteria?.examination_content}</p>
                    <p className="text-[7px] text-gray-300 font-black mt-1 uppercase">Evaluated: {new Date(h.updated_at).toLocaleDateString()}</p>
                  </div>
                </div>
              ))
            )
