...
export default function AdminDashboard() {
  ...
  async function fetchData() {
    try {
      ...
      // 【修正ポイント】roleでのフィルタを一旦外し、全ユーザーを取得してからJS側で分ける
      // これにより「roleカラムがない/名前が違う」ことによる400エラーを回避します
      ...
      // JS側でフィルタリング（roleがstudentのものだけ抽出）
      const studentList = (p || []).filter(user => user.role === 'student');
      
      // もしフィルタ後0人なら、確認用に全ユーザーを出す（デバッグ用）
      setStudents(studentList.length > 0 ? studentList : (p || []));
      setCriteria(c || [])
      setEvaluations(e || [])

    } catch (err: any) {
      console.error('Fetch error details:', err)
      setErrorMsg(`${err.message} (Hint: Check if 'profiles' table exists and has correct columns)`)
    } finally {
      setLoading(false)
    }
  }

  // --- (updateStudentInfo 以下の関数は共通) ---
  ...
  const resetPassword = async (student: Profile) => {
    ...
    if (error) alert('管理者権限が必要です。Service Roleの設定を確認してください。')
    else alert('更新完了。新しいパスワード: ' + newPass)
  }
  ...
  if (loading) return <div className="p-10 text-center font-black animate-pulse text-gray-400 italic">CONNECTING TO SEIKUKAI DB...</div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {errorMsg && (
          <div className="bg-red-50 border-2 border-red-200 p-6 rounded-[30px] mb-10 text-red-600">
            <h2 className="font-black mb-1 text-sm uppercase">Database Error</h2>
        ...
        <div className="space-y-10">
          {students.length > 0 ? (
                ...
                <div className="bg-gray-50/50 p-6 md:p-8 border-b border-gray-100 flex flex-wrap items-center justify-between gap-6">
                  <div className="flex items-center gap-6">
                    <div className="flex flex-col">
                      <span className="text-[9px] font-black text-gray-300 uppercase tracking-widest mb-1">Name</span>
                      <input 
                        type="text" 
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => setPreviewStudent(student)} className="px-4 py-2 bg-[#001f3f] text-white rounded-xl text-[10px] font-black uppercase tracking-tighter hover:scale-105 transition-all shadow-md">Preview</button>
                    <button onClick={() => resetPassword(student)} className="px-4 py-2 bg-gray-100 text-gray-600 rounded-xl text-[10px] font-black uppercase tracking-tighter hover:bg-gray-200 transition-all">Reset</button>
                    <button onClick={() => deleteStudent(student.id)} className="px-4 py-2 bg-red-50 text-red-600 rounded-xl text-[10px] font-black uppercase tracking-tighter hover:bg-red-100 transition-all">Remove</button>
                  </div>
                </div>
                      ...
                      return (
                        <div key={criterion.id} className="flex items-center gap-4 p-4 bg-gray-50 rounded-[24px] border border-gray-100">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-[8px] font-black text-gray-400 uppercase tracking-widest leading-none mb-1">{criterion.examination_type || 'SKILL'}</p>
            ...
            <div className="bg-white p-20 rounded-[50px] text-center border-2 border-dashed border-gray-200">
              <p className="text-gray-400 font-black uppercase tracking-widest">No Students Found</p>
            </div>
          )}
        </div>
      </div>
